...
class Tomarket {
    ...
    try {
      ...
      if (tiketsSpinFree > 0) {
        for (let i = 0; i < tiketsSpinFree; i++) {
          const result = await this.http(url_spin, this.headers, {
      ...
      if (rankDataRes.status === 200 && rankDataRes.data.status === 0) {
        if (rankDataRes.data.data.isCreated) {
          ...
          if (rankDataRes.data.data.unusedStars > 0 && this.autpUpgradeRank) {
            await this.http(upgradeRankUrl, this.headers, JSON.stringify({ stars: rankDataRes.data.data.unusedStars }));
          }
  ...
  async upgradeRank(stars) {
    const url = "https://api-web.tomarket.ai/tomarket-game/v1/rank/upgrade";
    const data = JSON.stringify({ stars });
    try {
      const res = await this.http(url, this.headers, data);
      ...
      this.log(colors.red(`Lỗi khi nâng cấp rank: ${error.message}`));
      return null;
    }
  }
  ...
  async claimPuzzle() {
    const puzzledaily = await this.getPuzzle();
  ...
  async manageTasks(initData) {
    ...
    for (const type of taskTypes) {
      if (type === "3rd") {
        await this.processTasks(tasks[type]["default"], type, initData);
      } else if (tasks[type]) {
  ...
  async main() {
    ...
    const dataFile = process?.env?.NAME_FILE_QUERY_ID_NOPROXY ? process?.env?.NAME_FILE_QUERY_ID_NOPROXY : "data.txt";
    ...
    while (true) {
      ...
      for (let i = 0; i < datas.length; i++) {
        ...
        try {
          const result = await this.processAccount(datas[i], i, this.enableTask);
          if (result !== null) {
